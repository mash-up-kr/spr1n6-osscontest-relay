...
class OutboxEndpointTest : RelayIntegrationTest() {
	...
	fun `republish makes the row immediately due`() {
		...
		// act() 는 republish() 직후 trigger.signal() 도 쏜다 (spec 대로). 이 클래스에서는
		// polling/listener 가 꺼져 있어도 DrainTrigger 자체는 살아 있으므로, statusOf(id) 로
		// "PENDING" 을 확인하면 실제 드레인 스레드와 경합해 PUBLISHING/PUBLISHED 로 이미
		// 넘어가 버릴 수 있다 (DrainTriggerTest 가 signal 후 awaitIdle 을 쓰는 이유와 같다).
		// republish 가 남기는 불변 사실 — 시도 횟수 리셋과 즉시 만기 — 을 행에서 직접 확인해
		// 드레인 진행 여부와 무관하게 "즉시 재발행 대상이 됐다" 를 검증한다.
		val row = jdbc.sql(
