...
--
-- SchemaValidator 검증 전용. 합의 이전 형태를 재현한다:
--   next_attempt_at 이 nullable 이고 이름도 다르며, status CHECK 에 DEAD 가 없다.
...
CREATE INDEX idx_outbox_pending ON outbox_event (next_retry_at, id) WHERE status = 'PENDING';
