...
--
-- SchemaValidator 검증 전용. "컬럼은 존재하지만 nullable" 형태를 재현한다:
-- next_attempt_at 컬럼 이름과 나머지 정의는 합의된 스키마와 동일하지만 NOT NULL 이 빠졌다.
-- outbox-broken.sql(컬럼 자체가 없거나 이름이 다른 경우)과 구분되는, SchemaValidator 의
-- "존재하지만 nullable" 분기 전용 픽스처다.
